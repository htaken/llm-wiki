...
dense probing（凍結バックボーン）で **PE・DINOv2・DINOv3・Sapiens(v1)** と比較し、同規模で上回る。Sapiens2-5B は DINOv3-7B を含む全ベースラインを全タスクで凌駕。姿勢では[[concepts/3D手姿勢推定|手]]を含む 308 点の全身推定で whole-body 姿勢推定器（ViTPose+/DWPose/RTMW）を上回る。
