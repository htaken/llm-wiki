...
sources: [AIxSuture:vision-based-assessment-of-open-suturing-skills.md, 2605.22200v1.md, 1-s2.0-S0039606025009316-main.md]
...
その後 [[sources/開放手術スキルの時空間特徴ML評価]]（Alipour et al. 2026）が、同じ3クラス設定（**novice n=119 / intermediate n=79 / proficient n=116**）で[[entities/CNN-LSTMハイブリッドモデル]]（fine-tuned [[entities/ResNet50]] + 双方向[[concepts/LSTM]] + アテンション）を適用し、**マクロF1 0.82**（AUC 0.95±0.009）を達成。AIxSuture原論文が報告したI3D・Video Swinの値を比較ベースラインとし、全スキルレベルで上回ったと主張している。なお同研究は1 fps・300フレーム固定サンプリング、512×512入力という原論文と異なる前処理を用いる。
