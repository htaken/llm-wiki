...
リポジトリ外への持ち出し用（research-plan/、同日ユーザー同意のうえ反映）:
- `research-plan/README.md` — Tier 1 4本の重要更新ブロック②／一行テーゼ改訂（分解の問い・公開ベンチマーク中心、#2 を分母へ降格）／「なぜこの方向か」更新／ロードマップで Stage 0 を publishable core 化／最大リスクに feasibility 知見（単眼の脆さ・top-down レシピ・Sapiens 弱・biomech 逆効果）
- `research-plan/01-related-work.md` — product↔process 地図に Atazadah・Cui を追加／「Tier 1 4本による4本柱の再評価」節／鋭い仮説に Atazadah の反証注意（スキル層 vs タスク複雑度の2予測）／feasibility 知見の更新節／一次ソースに4本追加
- `research-plan/02-stage0-experiments.md` — Stage 0a に「完了時間の product/process 切り分け」付随分析／プローブを top-down レシピ・Sapiens 期待値下げ・palm フォールバックで更新／成果物に完了時間帰属と publishable core 明記・2つの直交予測
- `research-plan/03-prior-art-prompt.md` — 既知文献リストに Tier 1 4本（ingest 済）を追加
